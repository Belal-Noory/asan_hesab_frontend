...
function AddCustomer() {
                ...
                data.some((el) => {
                    if (el.hasOwnProperty("name") || el.hasOwnProperty("phone")) {
                        data.forEach(async (element) => {
                            setUploading(true);
                            let res = await addCustomer(element.name, element.phone);
                            if (res) {
                                toast.current.show({
                                    severity: "success",
                                    summary: "موفقانه اجرا شد",
                                    detail: "مشتریان موفقانه در سیستم ثبت شدند",
                                });
                                setUploading(false);
                            }
                        });
    ...
    const saveCustomer = (e) => {
        ...
        if (singleCustomer.name.length > 3) {
            setSpinner(true);
            setformError(false);
            addCustomer(singleCustomer.name, singleCustomer.phone);
            toast.current.show({ severity: "success", summary: "موفق", detail: "مشتری موفقانه درج سیستم شد.", life: 3000 });
            setSpinner(false);
        ...
        <div className="grid">
            <div className="col-12 md:col-6 text-right">
                <div className="card bg-primary">
                    ...
                    <p>مشتری محترم حالا شما میتوانید تمام مشتریان شرکت خود را در یک فایل Excel ذخیره کرده و به سیستم ما اپلود کونید. قابل توجه است که فایل Excel شما باید ستون ها یا کالم های name و phone را داشته باشد.</p>
            ...
            <div className="col-12 md:col-6" dir="rtl">
                <div className="card p-fluid">
                    <h5 className="mx-3">ثبت مشتری از طریق فورم</h5>
                    <p>مشری محترم شما میتوانید مشریان خود را یا از طریق فورم ذیل در سیستم ثبت کنید و یا هم لیست از مشتریان تانرا در یک فایل excel ذخیزه کرده و به سیستم اپلود کنید.</p>
                    <div className="field col-12">
                        <span className="p-float-label">
                            ...
                            <label htmlFor="name">اسم مشتری</label>
                        </span>
                    </div>
                    <div className="field col-12">
                        <span className="p-float-label">
...
export default React.memo(AddCustomer, comparisonFn);
